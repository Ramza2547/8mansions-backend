...
function DataPage() {
  ...
  const trackChanges = (original, edited) => {
    const changes = [];
    if (original.name !== edited.name) changes.push({ field: 'Name', before: original.name || '-', after: edited.name });
    if (original.nationality !== edited.nationality) changes.push({ field: 'Nationality', before: original.nationality || '-', after: edited.nationality });
    if (original.date_of_birth !== edited.date_of_birth) changes.push({ field: 'Date of Birth', before: formatDate(original.date_of_birth), after: formatDate(edited.date_of_birth) });
  ...
  // 🎯 2. ประกอบร่างข้อมูลทั้งหมดให้เป็นก้อนเดียวกันเพื่อง่ายต่อการ Search
  ...
  // 🎯 3. ระบบกรองข้อมูล (Filter) เมื่อมีการพิมพ์ลงในช่องค้นหา
  const filteredRooms = allRoomsData.filter(item => {
    const search = searchTerm.toLowerCase();
    // ค้นหาได้ทั้ง เลขห้อง, ชื่อลูกค้า, หรือสัญชาติ
    ...
    <div className="flex flex-col min-h-screen bg-[#EAEAEA] relative font-sans">
      ...
      <div className="flex justify-center flex-1 py-8 sm:py-12 px-4 sm:px-10">
        <div className="w-full max-w-5xl">
          ...
          {/* 🎯 4. กล่อง Search Bar สวยๆ ด้านบน */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
            {/* 🎯 5. เอา filteredRooms มาแสดงผลแทน roomNames แบบเดิม */}
            {filteredRooms.length > 0 ? (
                ...
                <div key={index} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-5 sm:p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border-l-4 border-transparent hover:border-[#8FAFC1]">
                  ...
                  <div className="text-[14px] sm:text-[15px] text-[#1A1A1A] leading-relaxed mb-4 sm:mb-0 w-full pr-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-2 gap-y-1">
                      <div><span className="font-semibold text-gray-600">Name:</span> <br/>{data.displayName}</div>
                      <div><span className="font-semibold text-gray-600">Nationality:</span> <br/>{data.displayNationality}</div>
              ...
              // 🎯 กรณีพิมพ์ค้นหาแล้วไม่เจอข้อมูลเลย
      ...
      {/* ... โค้ด Modal Edit และ Modal History อยู่เหมือนเดิมทุกประการครับ ... */}
      {editingCustomer && (
